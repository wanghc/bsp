   ...
   */
   ...
   + <details>
     ...
     <li>@param {Object} options [可选] 打印配置信息
     	  ...
     	  <tr><td>PFlag</td><td>XML模板代码,打印方法自动加载XML模板内容。<br/>一次打印多个模板的时候，必传此配置值，避免模板覆盖的风险 <code style='color:red;'>20250805</code></td></tr>
     	  <tr><td>langCode</td><td>现PFlag一起使用，指定XML模板对应的语言代码。<br/>如果未定义对应语种模板时，使用原模板打印。如：EN <code style='color:red;'>20250805</code></td></tr>
     	  </table>
     </li></ul></pre>
     </details>
...
- IE11浏览器下得到电子票据base64时，偶尔带有绿色块与黑色块时，可以通过`otherCfg.retryMaxTimeByImgGreen`参数控制重新生成次数 [6854948]

  ```js
  otherCfg.retryMaxTimeByImgGreen = 2; // 出问题后,再最多重试二次
  myprexmlpreview.init(inpara,inlist,[],reportNote,otherCfg);
  ```
