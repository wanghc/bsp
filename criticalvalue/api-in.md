...
- [2. 接口列表](#2-接口列表)
  ...
  - [2.41 根据就诊获取危急值报告数量](#241-根据就诊获取危急值报告数量)
...
| ReportInfo   | 报告审核信息    | 审核日期2(yyyy-MM-dd)^审核时间2(hh:mm:ss)^审核人ID@审核人工号@审核人姓名3^审核说明4^报告审核科室5^报告科室联系电话6^危急值等级7（Normal一般危急值  High高危危急值 Extreme极危危急值 Unknown未分级）<br>`审核人`可以理解为危急值上报人，指的是确定危急值要报告的人，对于需要审核才上报的是审核人，但是对于不需要审核就上报的那是报告人。<br>某些项目上由于用户不互通，医技系统的用户工号在HIS内不存在，审核人部分支持“id@工号@姓名”形式(`HIS9.1.3`) |
...
```vb
##class(web.DHCAntCVService).GetCVReportNum(EpisodeId, Status, FlagOnly, AppUserId, RepTimeRange, pStatus)
```
...
| -------------- | ----------------- | ------------------------------------------------------------ |
| EpisodeId   | 就诊ID    |  |
| Status   | 状态条件    | 空（除了撤销外的） C未接收处理 F已处理 Rec已接收未处理 D已撤销  <br> `8.4.2`后支持多个`,`分隔 |
| FlagOnly   | 是否仅查标志    | 0-查所有  1-仅查标志，即查到一条就返回 |
| AppUserId   | 申请用户ID   | 可为空 <br> 检验/检查申请医生用户ID <br> `8.4.2` |
| RepTimeRange   | 报告时间范围   | 可为空 <br> 格式为"开始时间,结束时间",如"2023-11-20 00:00:00,2023-11-22 09:00:00" <br> `9.0.2` |
| pStatus   |  更细的状态条件   | 可为空（非必要情况建议传空） <br>  为`^`分隔形式  pRecStatus^pExecStatus^pEmrStatus <br> 1-pRecStatus:接收状态  及时接收1 超时接收2  未接收但未超时3 超时未接收4<br>2-pExecStatus:处理状态  及时处理1 超时处理2  未处理但未超时3 超时未处理4  <br>3-pEmrStatus:病历状态 及时书写1 超时书写2  未书写但未超时3 未书写已超时4 <br> `9.0.2` |

|*返回值* |*说明*|*备注*|
| --- | -- | -- |
| 数字 | 查到的数量 | FlagOnly=1时只会返回0或1 |