...
- [消息平台](#消息平台)
	...
	- [2. 消息处理](#2-消息处理)
		...
		- [2.3 消息撤销接口Cancel](#23-消息撤销接口cancel)
				- [OtherParams以^分隔每个位置说明](#otherparams以分隔每个位置说明-1)
	- [3. 配置说明](#3-配置说明)
		- [3.1 接收对象](#31-接收对象)
	...
	- [4. 其它接口](#4-其它接口)
		...
		- [4.5 消息确认接口](#45-消息确认接口)
				- [确认方法OtherParams参数以^分隔每个位置说明](#确认方法otherparams参数以分隔每个位置说明)
	...
	- [5. 常见问题](#5-常见问题)
		- [5.1 消息变为已处理的几种方式](#51-消息变为已处理的几种方式)
			...
			- [5.1.2 需要处理](#512-需要处理)
				- [5.1.2.1 Exec接口（最正统推荐的）](#5121-exec接口最正统推荐的)
				- [5.1.2.2 ExecAll接口](#5122-execall接口)
				- [5.1.2.3 工具按钮【执行按钮】](#5123-工具按钮执行按钮)
				- [5.1.2.4 点击按钮弹窗即处理（HIS8.5.3+）](#5124-点击按钮弹窗即处理his853)
				- [5.1.2.5 Cancel接口](#5125-cancel接口)
				- [5.1.2.6 通用处理链接处理](#5126-通用处理链接处理)
			- [5.1.3 自动处理](#513-自动处理)
				- [5.1.3.1 出院自动处理](#5131-出院自动处理)
				- [5.1.3.2 消息超期自动处理](#5132-消息超期自动处理)
