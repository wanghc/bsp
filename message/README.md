...
|ToLocRowId     | 接收消息的科室 Id | 可以为空。<br>格式"LocId1^LocId2^LocId3\|其它标记" <br> "1^2^3" 发给科室1、2、3所有医护人员<br> "1^2^3\|ToNurse"发给科室1、2、3所有护士<br>"1^2^3\|ToDoctor"发给科室1、2、3所有医生<br>"1^2^3\|Logon"发给有此科室1或2或3登录权限的所有用户(HIS8.4)<br>"1^2^3\|OnlyFlag"仅作为一个标识，标识此消息应该登录哪个科室才能看到，具体接收用户还需要通过ToUserRowId参数传<br>此参数主要是为了解决一个发给A科的所有人的消息（比 如会诊），<br>但是某人拥有AB两科的权限，在登录B科时 不查看 A科消息 |
...
| ActionTypeArg   | 消息类型代码    | 如果为空 不限制消息类型，多个消息类型代码用英文`,`分隔（`9.2.0`支持多个） |
...
```vb
w ##class(BSP.MSG.SRV.QryInterface).GeCountByAdm(pAdm, pFlagOnly, pActionCodes, pExecFlag)
```

```vb
/// 获取患者标本拒收1006未处理的数量 
w ##class(BSP.MSG.SRV.QryInterface).GeCountByAdm(1192,0,"1006","N")

/// 获取患者是否有标本拒收1006未处理 
w ##class(BSP.MSG.SRV.QryInterface).GeCountByAdm(1192,1,"1006","N")
```

| *参数名* | *说明*      | *备注*                                                 |
| -------------- | ----------------- | ------------------------------------------------------------ |
| pAdm   | 就诊ID    | 就诊ID 必须 |
| pFlagOnly   | 是否仅获取标志 0/1    | 1=仅查标志，如果存在一条就返回1，不再继续查询 <br> 0=查询具体数量 |
| pActionCodes   | 消息类型代码    | 多个用逗号分隔 |
| pExecFlag   | 处理状态标志    | 消息内容表的处理标志 空不限制 Y已处理  N未处理 <br>注意消息的处理标志不总是能代表业务的处理标志，如消息出院自动处理，危急值转发时处理危急值消息等 |
